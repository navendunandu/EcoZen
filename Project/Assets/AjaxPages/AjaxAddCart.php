...
if($result->num_rows>0)
{
	...
	$row=$res->fetch_assoc();
	$bid = $row["id"];
    $selqry="select * from tbl_cart where booking_id='".$bid."'and product_id='".$_GET["fid"]."' and cart_status='0'";
    $result=$con->query($selqry);
    if($result->num_rows>0)
    {
    ...
    {
        $insQry1="insert into tbl_cart(product_id,booking_id)values('".$_GET["fid"]."','".$bid."')";
